...
if(isset($idget)){
    $delete = mysqli_query($connect, "DELETE FROM `Plans` WHERE id = '$idget'");
    if($delete){
        echo swal('Thành Công','success');
        echo redirect('./addpackage.php');
    } else {
...
if(isset($_POST['submit'])){
    ...
    $price = $_POST['price'];
    $ssl = $_POST['ssl'];
   $intrue = mysqli_query($connect, "INSERT INTO `Plans`(`id`, `dungluong`, `ssl`, `bangthong`, `mienkhac`, `bidanh`, `maychu`, `backup`, `planapi`, `price`) VALUES (NULL,'$disk','$ssl', '$bangthong','$addon','$sub','$maychu','$bk','$package','$price')");
...
<div class="card-header">
 <h3 class="card-title"> Thêm Gói Hosting </h3>
</div>
...
<input type="text" class="form-control" name="bangthong">
</div>

<div class="form-group">
<label for="exampleInputEmail1"> Miền Khác </label>
<input type="text" class="form-control" name="addondomain">
</div>
...
<input type="text" class="form-control" name="ssl">
</div>
