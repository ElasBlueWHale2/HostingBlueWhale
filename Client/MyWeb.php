...
echo Title('Danh Sách Trang Web');
?>
...
<h2 class="text-lg font-medium mr-auto"> Quản Lý Trang Web </h2>
</div>

<div class="intro-y box mt-5">
<div class="flex flex-col sm:flex-row items-center p-5 border-b border-slate-200/60">
<h2 class="font-medium text-base mr-auto"> Danh Sách Web </h2>
</div>
...
<tbody>
    ...
    <?php
    $danhsach = mysqli_query($connect, "SELECT * FROM DataWeb WHERE username = '".$getUser['username']."'");
...
<td class="whitespace-nowrap"><?=ToTime($row['timeend']);?></td>
<td class="whitespace-nowrap"> <?=StatusHosting($row['status']);?> </td>
<td class="whitespace-nowrap">
<button class="btn btn-pending" onclick="window.location.href='/dataweb/<?=$row['id'];?>';"> Quản Lý </button>
</td>
</tr>
